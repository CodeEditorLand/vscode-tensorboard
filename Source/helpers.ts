...
export function containsTensorBoardImport(
	...
	try {
		for (const s of lines) {
			if (
				s &&
				(TensorBoardImportRegEx.test(s) ||
					TorchProfilerImportRegEx.test(s))
			) {
				return true;
			}
		}
...
export function containsNotebookExtension(
	...
	for (const s of lines) {
		if (
			s?.startsWith("%tensorboard") ||
			s?.startsWith("%load_ext tensorboard")
		) {
			return true;
		}
	}
...
export function hasFileBasedWorkspace() {
	if (
		...
		workspace.workspaceFolders.some((w) => w.uri.scheme === "file")
	) {
		return true;
	}
